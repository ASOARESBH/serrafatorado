<?php
/**
 * API Administrativa para Gerenciamento de Fornecedores
 * 
 * Permite listar todos os fornecedores e ativar/inativar
 * 
...
function listarTodos($conexao) {
    ...
    while ($row = mysqli_fetch_assoc($resultado)) {
        // Formatar dados
        $row['media_avaliacoes'] = number_format($row['media_avaliacoes'], 1);
        $row['data_cadastro_formatada'] = date('d/m/Y H:i', strtotime($row['data_cadastro']));
        $row['ultimo_acesso_formatado'] = $row['ultimo_acesso'] ? date('d/m/Y H:i', strtotime($row['ultimo_acesso'])) : 'Nunca';
...
/**
 * Buscar dados de um fornecedor específico
 */
function buscarFornecedor($conexao) {
    ...
    $sql = "SELECT * FROM v_fornecedores_completo WHERE id = $id";
    $resultado = mysqli_query($conexao, $sql);
    
    if (!$resultado) {
        resposta(false, 'Erro ao buscar fornecedor: ' . mysqli_error($conexao));
    }
    ...
    $fornecedor = mysqli_fetch_assoc($resultado);
    resposta(true, 'Fornecedor encontrado', $fornecedor);
}
...
function obterEstatisticas($conexao) {
    ...
    $stats_pedidos = mysqli_fetch_assoc($resultado_pedidos);
    
    $stats = array_merge($stats, $stats_pedidos);
